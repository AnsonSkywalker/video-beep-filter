...
**自动检测视频音频中的「六四」「八九」等敏感数字收款语音播报，并用「哔——」声替代，帮助录播视频过审。**
...
```bash
python beep_filter.py "D:\家是本录播\B\录制.flv"
```
...
> ⚡ **GPU 加速**：如果您有 NVIDIA GPU（如 RTX 4070 Ti SUPER），可以安装 CUDA 版 PyTorch 获得 3-5 倍加速：
> ```bash
...
**A:** 脚本使用 `-c:v copy` 直接复制视频流，**不重新编码**，所以文件大小几乎不变。音频轨从原始格式重新编码为 AAC 192kbps。
